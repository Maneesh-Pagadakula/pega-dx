...
function extractGroupId(groupId) {
  ...
  if (Array.isArray(groupId)) textToExtractFrom = groupId[0];
  else if (typeof groupId === 'string') textToExtractFrom = groupId;
...
test.describe.serial('UCC Search False - Search & Navigate Only', () => {
  ...
  test('UCC Search - False API + Navigate to Case', async () => {
    const getOrderDetailsAPI = env.getAPIList().get('Get Order Details');
    const uccSearchAPI = env.getAPIList().get('UCC Search - False');
    const getCaseDetailsAPI = env.getAPIList().get('Get Case Details');

    const runtimeData = {
      orderId: 'O-278466' // Replace or fetch dynamically
    };

    const result = await uccSearchFalseWorkflow.executeGetOrderDetails(getOrderDetailsAPI, runtimeData);
    if (!result?.id) {
      console.warn('[FAIL] No valid Order ID from Get Order Details API. Skipping test.');
      test.skip();
    }
    ...
    await uccSearchFalseWorkflow.executeUCCSearchFalse(uccSearchAPI, runtimeData);
    await uccSearchFalseWorkflow.UCCSearchUIValidation(orderId, groupId);
  });
});
